...
var Database = function() {
        ...
        return new Promise((resolve, reject) => {
            db.transaction(function(tx) {
                tx.executeSql("INSERT INTO todo(item, created, status, priority)" +
                    " VALUES (?, ?, ?, ?)", [item.item, item.created, item.status, item.priority],
    ...
    this.updateStatus = function(id, status) {
        ...
        return new Promise((resolve, reject) => {
            db.transaction(function(tx) {
                tx.executeSql("UPDATE todo SET status = ? WHERE id = ?", [status, id],
                    (tx, results) => { resolve(results); },
                    (tx, error) => { reject(error); }
                );
            });
    ...
    this.deleteByKey = function(item) {
        ...
        return new Promise((resolve, reject) => {
            db.transaction((tx) => {
                tx.executeSql("DELETE FROM todo WHERE id = ?", [item.id],
                    (tx, results) => { resolve(results); },
                    (tx, error) => { reject(error); });
            });
        });
